...
struct MCTSActorParams {
  ...
  std::string info() const {
    ...
    ss << "[name=" << actor_name << "][ply_pass_enabled=" << ply_pass_enabled
       ...
       << "][rotation_flip=" << rotation_flip << "][komi=" << komi << "]" << std::endl;
    ss << "coord            reward/num_visits(Q),       prior_probability,    child node id" << std::endl;
    return ss.str();
  }
};
  ...
  static void pi2response(
    ...
    while (true) {
      ...
      if (valid) {
       ...
       if (oo != nullptr) {
        *oo << "Predict [" << i << "][" << coord2str(v.first) << v.second;
        if (valid)
          *oo << " added" << std::endl;
        else
          *oo << " invalid" << std::endl;
